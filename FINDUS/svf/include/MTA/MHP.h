...
public:
    ...
    //      (td->handle_set.find(op1)!=td->handle_set.end())
    // }
    ...
    inline CallICFGNode* getCBN(const SVFInstruction* inst)
    {
        // llvm::outs()<<" -----get CNB----\n";
            ...
            auto c = tct->getCallICFGNode(inst);
        //  llvm::outs()<<" -----after get CBN----\n";
        return c;
    }
    ...
    inline const PTACallGraph::FunctionSet& getCallee(const SVFInstruction* inst, PTACallGraph::FunctionSet& callees)
    {
        //  llvm::outs()<<"PTACallGraph::FunctionSet  getCallee called \n";
        getTCG()->getCallees(getCBN(inst), callees);
            // llvm::outs()<<"PTACallGraph::FunctionSet  getCallee end \n";
        return callees;
    }
    /// ThreadCallGraph
