        ...
        if(ObjVar* objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii)))
        {
            ...
            if(obj->isFunction())
            {
                ...
                if(SVFUtil::matchArgs(cs->getCallSite(), callee) == false)
                    continue;
    //    outs()<<" SSSSSSSSSSSSSSSSSSSSSSS resolving indirect call edge: TTTTTTTTTTTTTTTTTTTTTTTTT ";
                if(0 == getIndCallMap()[cs].count(callee))
                {
                    newEdges[cs].insert(callee);
                    getIndCallMap()[cs].insert(callee);

                    // outs()<<" TTTTTTTTTTTTTTTTTTTT resolving indirect call edge: TTTTTTTTTTTTTTTTTTTTTTTTT "
                        //  << cs->getCallSite()->toString()
                        //  << " to callee: " << callee->getName() << "\n";
