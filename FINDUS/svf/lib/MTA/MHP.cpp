 ...
 void MHP::analyzeInterleaving()
 {
     for (const std::pair<const NodeID, TCTNode*>& tpair : *tct)
     {
         ...
         while (!cxtStmtList.empty())
         {
            ...
            //  cts.dump();
            //  outs() << "current thread interleaving: < ";
            //  outs() << " >\n-----\n";
             DBOUT(DMTA, outs() << "-----\nMHP analysis root thread: " << rootTid << " ");
             DBOUT(DMTA, cts.dump());
             ...
             else
             {
                 ...
                 {
                   // llvm::outs()<<" ******enter MHP handleRet***********\n";
                     handleRet(cts);
                 }
                 else
                 {
                   // llvm::outs()<<" ******enter MHP handleIntra***********\n";
                     handleIntra(cts);
                 }
             }
         }
...
//  std::cout<<" ******enter MHP handleJoin***********\n";
 ...
 void MHP::handleCall(const CxtThreadStmt& cts, NodeID rootTid)
 {
     ...
     const SVFInstruction* call = cts.getStmt();
     const CallStrCxt& curCxt = cts.getContext();
     //llvm::outs()<<" --------MHp cnb before-------\n";
     CallICFGNode* cbn = getCBN(call);
       //   llvm::outs()<<" --------MHp cnb after-------\n";
 ...
 void ForkJoinAnalysis::analyzeForkJoinPair()
 {
     for (const std::pair<const NodeID, TCTNode*>& tpair : *tct)
     {
         ...
         if (const SVFInstruction* forkInst = ct.getThread())
         {
             ...
             const InstVec& nextInsts = forkInst->getSuccInstructions();
            // std::cout<<" the current thread instruction is ------------->   "<<forkInst->toString()<<" \n\n\n";


                    auto* td = tct->getTCTNode(rootTid);
                    // const SVFCallInst *V = dynamic_cast<const SVFCallInst*>(call);
                    const Value *V1 = LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(forkInst);
                    // llvm::outs()<<*V1<<" ------------------->";
                    llvm::Value  *V = const_cast<Value*>(V1);
                    llvm::CallInst* callL = llvm::dyn_cast< llvm::CallInst>(V);
                    // llvm::outs()<<"###########################################"<<V->getValueID();
                    llvm::Value * op1 = callL->getOperand(0);

                    while(!llvm::isa<llvm::AllocaInst>(op1) &&  !llvm::isa<llvm::GlobalVariable>(op1))
                    {
                        if(llvm::isa<llvm::LoadInst>(op1)   )
                        {
                            op1 = llvm::dyn_cast<llvm::LoadInst>(op1)->getOperand(0);
                            // llvm::outs()<<*op1<<" inside loop \n";
                        }else{
                            break;
                        }
                        // llvm::outs()<<*op1<<" inside outside loop \n\n";
                    }
                if(op1)
                {
                    // llvm::outs()<<" ---------is inserting----------- \n"<<*op1<<"\n";
                    td->handle_set.insert(op1);
                    td->handle=op1;
                }
                else{
                    // llvm::outs()<<" why is not inserting \n";
                }

          //   std::cout<< "-----  analyzeForkJoinPair  the work list is starting ------ \n";
        ...
        // std::cout<< "\n\n\n-----  analyzeForkJoinPair  the work list is starting ------ \n";
             ...
             while (!cxtStmtList.empty())
             {
                ...
                //  std::cout<< "statements popping from the cxtstmtList are  "<<curInst->toString()<<" \n";
                 ...
                 else if (SVFUtil::isCallSite(curInst) &&  !isExtCall(curInst)   &&  tct->isCandidateFun(getCallee(curInst, callees)))
                 {
                     handleCall(cts, rootTid);
                 }
                 else if (curInst->isRetInst())
 ...
 {
     const SVFInstruction* call = cts.getStmt();
    //  llvm::outs()<<"---------------------- "<<call->toString()<<"----------- this is important -----------------";
     const CallStrCxt& curCxt = cts.getContext();
     assert(isTDFork(call));
     CallICFGNode* cbn = getCBN(call);
 ...
 void ForkJoinAnalysis::handleJoin(const CxtStmt& cts, NodeID rootTid)
 {
     ...
     if (getTCG()->hasCallGraphEdge(cbn))
     {
        ...
        //  std::cout<<" the join site is "<<joinSite->toString()<<" \n";
          auto *td = tct->getTCTNode(rootTid);
            ...
            const Value *V1 = LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(joinSite);
            Value  *VV = const_cast<Value*>(V1);
            llvm::CallInst *callL = llvm::dyn_cast<llvm::CallInst>(VV);
    ...
    while(!llvm::isa<llvm::AllocaInst>(op1) &&  !llvm::isa<llvm::GlobalVariable>(op1))
    {
        ...
        // llvm::outs()<<*op1<<" \n";
         ...
         if (isAliasedForkJoin(forkSite, joinSite)  ||   (td->handle_set.find(op1) != td->handle_set.end())  )
         {
         ...
         else
         {
            ...
            // for(llvm::Value * x : td->handle_set)
            // {
            //     llvm::outs()<<*x<<"---";
            // }
 ...
 void ForkJoinAnalysis::handleCall(const CxtStmt& cts, NodeID rootTid)
 {
    ...
    //   std::cout<<"************************ handling the call  stmt ************** \n";
     auto *td = tct->getTCTNode(rootTid);

        // llvm::outs()<<td->getCxtThread().getThread()->toString()<<" Nothing got printed "<<*(td->handle);
     auto handle =td->handle;


     const SVFInstruction* call = cts.getStmt();


    // llvm::CallInst *callL = const_cast<llvm::CallInst*>(llvm::dyn_cast<llvm::CallInst>(call));
     ...
     if (getTCG()->hasCallGraphEdge(cbn))
     {
         for (PTACallGraph::CallGraphEdgeSet::const_iterator cgIt = getTCG()->getCallEdgeBegin(cbn),
            ...
            const SVFFunction* svfcallee = (*cgIt)->getDstNode()->getFunction();
            const Value *V1 = LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(call);
            Value  *VV = const_cast<Value*>(V1);
             llvm::CallInst *callL = llvm::dyn_cast<llvm::CallInst>(VV);
      

            const Value *f1 = LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(svfcallee);

            auto  *ff = const_cast<Value*>(f1);

             llvm::Function *ffun = const_cast<llvm::Function*>(llvm::dyn_cast<llvm::Function>(ff));
           
             int ci =0;
            for (llvm::Value *arg : callL->args()) {

                while(! llvm::isa<llvm::AllocaInst>(arg) && !llvm::isa<llvm::GlobalVariable>(arg) )
                {
                    if(llvm::isa<llvm::LoadInst>(arg))
                    {
                        arg = llvm::dyn_cast<LoadInst>(arg)->getOperand(0);
                    }
                }
                if(arg == handle)
                {
                    td->handle_set.insert(ffun->getArg(ci));
                }
                else{
                    // llvm::outs()<<"arg is not matching dude "<<*arg<< ;
                }
                ci++;
                }
                ...
                for (auto &BB : *ffun) {
                for (auto &Inst : BB) {
                if (llvm::StoreInst *CI = llvm::dyn_cast<llvm::StoreInst>(&Inst)) {
                    ...
                    if(td->handle_set.find(op1)!= td->handle_set.end())
                    {

                        td->handle_set.insert(op2);
                    }
 ...
 void ForkJoinAnalysis::handleIntra(const CxtStmt& cts)
 {
     ...
     const SVFInstruction* curInst = cts.getStmt();
        ...
        // llvm::Instruction *ii = const_cast<llvm::Instruction*>(llvm::dyn_cast<llvm::Instruction>(V));
     ...
     while (!worklist.empty())
     {
         ...
         for (TCT::ThreadCreateEdgeSet::const_iterator it = tct->getChildrenBegin(node), eit = tct->getChildrenEnd(node); it != eit; ++it)
         {
             ...
             if (isFullJoin(tid, childTid))
             {
                 ...
                 //   std::cout<<" the thread is not fully joined \n"<<childTid<<"\n";
             }
         }
