...
void customfunction(){
     ...
     mongo::executor::ThreadPoolExecutorTest t1;
    mongo::executor::ThreadPoolTaskExecutor & executor = t1.getExecutor();

    t1.launchExecutorThread();
    auto status1 = t1.getDetectableErrorStatus();
    mongo::unittest::Barrier barrier{2};
    executor.schedule1([&](mongo::Status status) {
        status1 = status;
        barrier.countDownAndWait();
    });
    barrier.countDownAndWait();
