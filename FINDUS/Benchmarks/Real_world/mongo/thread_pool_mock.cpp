...
void* ThreadPoolMock::_workerThreadFunction(void* arg) {
    ThreadPoolMock* pool = static_cast<ThreadPoolMock*>(arg);
    
   // pool->_options.onCreateThread();
    stdx::unique_lock<Latch> lk(pool->_mutex);

  //  LOGV2_DEBUG(22603, 1, "Starting to consume tasks");
    // while (!pool->_joining) {
        // if (pool->_tasks.empty()) {
        //     lk.unlock();
        //     pool->_net->waitForWork();
        //     lk.lock();
        //     continue;
        // }

        pool->_consumeOneTask(lk);
    // }
...
void ThreadPoolMock::startup() {
    ...
    //     LOGV2_DEBUG(22604, 1, "Done consuming tasks");
    // });
    int result = pthread_create(&_worker, nullptr, _workerThreadFunction, this);
}
...
void ThreadPoolMock::_consumeOneTask(stdx::unique_lock<Latch>& lk) {
    ...
    // }
     std::function<void (Status )> fn = _tasks1;
    // _tasks.pop_back();
    // lk.unlock();
    // if (_inShutdown) {
