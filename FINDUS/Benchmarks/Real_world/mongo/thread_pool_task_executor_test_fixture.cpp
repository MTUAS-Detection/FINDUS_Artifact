...
// }
ThreadPoolTaskExecutor* makeThreadPoolTestExecutor1(
    NetworkInterfaceMock* net, ThreadPoolMock::Options options) {
    auto netPtr = net;
    auto * xx =new ThreadPoolMock(netPtr, 1, std::move(options));
   auto *yy= new ThreadPoolTaskExecutor(
        xx, net);
...
// std::unique_ptr<ThreadPoolTaskExecutor> ThreadPoolExecutorTest::makeTaskExecutor(
...
// }
ThreadPoolTaskExecutor * ThreadPoolExecutorTest::makeTaskExecutor1(
    NetworkInterfaceMock* net) {
    auto options = makeThreadPoolMockOptions();
    auto* pp = makeThreadPoolTestExecutor1(net, std::move(options));
    return pp;
}
}  // namespace executor
}  // namespace mongo
